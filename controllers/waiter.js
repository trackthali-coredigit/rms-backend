...
const waiterOrderList = async (req, res) => {
	try {
		...
		res.status(200).json({
			Status: 1,
			message: "WaiterOrderList get successfully",
...
const waiterOrderDetails = async (req, res) => {
	try {
		...
		res.status(200).json({
			Status: 1,
			message: "WaiterOrderDetails get successfully",
			OrderDetails,
		});
	} catch (error) {
		console.error("Error fetching order list:", error);
...
const waiterOrderComplete = async (req, res) => {
	try {
		...
		if (order.special_guest) {
			...
			await sendNotification([order.user_id], `your order has been complete!`, {
				order_id: req.body.order_id,
				notification_type: "Order Complete",
			});
		}
		res
			.status(200)
			.json({ Status: 1, message: "Order marked as paid successfully" });
	} catch (error) {
		console.error("Error accepting order:", error);
		res.status(500).json({ Status: 0, message: "Internal Server Error" });
	}
...
const waiterAssignedTableList = async (req, res) => {
	try {
		console.log(
			">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>IM IN waiterAssignedTableList api"
		);
