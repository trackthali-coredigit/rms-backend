...
const signin = async (req, res) => {
  try {
    console.log("1");
    const {emailOrUsername, password,device_id,device_type,device_token,role} = req.body;
    ...
    let deviceDetail = {device_id,device_type,device_token,user_id: user.user_id}
    ...
    if (role === "admin") {
     ...
     const otp = `1234`;
      ...
      return res.status(200).json({Status: 1,message: "OTP sent to your registered email successfully",otp: otp,user_id: user.user_id,});
    }
    ...
    if (role === "user" && !user.is_verify) {
      ...
      return res.status(200).json({Status: 2,message: "Please verify your Account First",token, otp,user_id: user.user_id,});
    }

    if (role === "user" && !user.is_account_setup) return res.status(200).json({Status: 3,message: "Please setup your Account First",token,user_id: user.user_id});
    res.status(200).json({Status: 1,message: "Sign-in successfully",token,user_id: user.user_id});
  } catch (error) {
...
const StaffForgetPassword = async (req, res) => {
  try {
    ...
    switch (req.userData.role) {
      case "admin":
        if (user.role == "admin" || user.role == "super_admin" || user.business_id != req.userData.business_id) {
          return res.status(404).json({ Status: 0, message: "The User Not uthorize for this action" });
        }

        break;
      case "supervisor":

        if (user.role == "admin" || user.role == "super_admin" || user.role == "supervisor" || user.business_id != req.userData.business_id) {
          return res.status(404).json({ Status: 0, message: "The User Not uthorize for this action" });
        }
        break;
      default:
...
const changePassword = async (req, res) => {
  try {
    ...
    if (oldPassword === newPassword) return res.status(400).json({ Status: 0, message: "New password must be different from the old password" });

    const user = req.userData;

    const isPasswordValid = await bcrypt.compare(oldPassword, user.password);
    if (!isPasswordValid) return res.status(200).json({ Status: 0, message: "Invalid Current password" });
    
    if (oldPassword === newPassword) return res.status(200).json({ Status: 0,message: "New password must be different from the old password",});
  ...
  try {
    ...
    if (invalidFileType) {
      return res.status(400).json({
        Status: 0, message: "Invalid file type. Only jpg, jpeg, and png are allowed",
      });
    }
    if (invalidFileSize) {
      return res.status(400).json({ Status: 0, message: "Image size must be less than 10MB" });
    }
    ...
    if (!!authHeader) {
      ...
      const userx = await db.User.findOne({
        where: {
          [Op.and]: [
            { user_id },
            { [Op.or]: [{ role: "admin" }, { role: "supervisor" }, { role: "user" },], },
          ],
        },
      });
      if (!userx) {
...
const getStaffMemberDetail = async (req, res) => {
  try {
    ...
    const staffMemberId = req.body.staffMemberId; // Assuming staff member ID is in the URL params
...
const deleteStaff = async (req, res) => {
  try {
    ...
    console.log(req.userData.role != "admin" && req.userData.role != "supervisor");
    ...
    if (!staffMember) {
      return res.status(404).json({ Status: 0, message: "The Staff member not found" });
    }
...
const waitersTableList = async (req, res) => {
  try {
    ...
    if (!waiter) {
      return res.status(404).json({ Status: 0, message: "The Waiter not found" });
    }
    const pageSize = 20;
...
const is_admin = async (req, res) => {
  try {
    if (req.userData.role != "admin" && req.userData.role != "supervisor") {
      return res.status(400).json({ Status: 0, message: "You are not authorized to perform this action" });
    }
...
const addPromoCode = async (req, res) => {
  try {
    ...
    if (existingPromoCode && (existingPromoCode.code === req.body.code) ) {
      return res.status(400).json({ Status: 0, message: 'Promo code already exists' });
    }
    ...
    return res.status(200).json({ Status: 1, message: 'Promo code added successfully', promoCode: newPromoCode });
  } catch (error) {
...
const getPromoCodes = async (req, res) => {
  try {
    ...
    const promoCodes = await Promise.all(rows.map(promoCode => {
...
const deletePromoCode = async (req, res) => {
  try {
    ...
    const promoCode = await db.PromoCode.findOne({ where: { promoCode_id, business_id } });
    if (!promoCode) {
      return res.status(404).json({ Status: 0, message: "Promo code not found" });
    }

    await promoCode.destroy();

    return res.status(200).json({ Status: 1, message: "Promo code deleted successfully" });
  } catch (error) {
...
const orderHistory = async (req, res) => {
  try {
    ...
    const totalPages = Math.ceil(count / pageSize);
    res.status(200).json({
      Status: 1, message: "OrderDetails get successfully",
      current_page: currentPage,
      total_pages: totalPages,
      total_amount,
      OrderDetails: rows
  ...
  try {
    ...
    if ((req.userData.role == "admin") || (req.userData.role == "supervisor")) {
      const orderTotalAmount = await db.Order.findAll({
        where: condition,
        attributes: [[db.sequelize.fn('SUM', db.sequelize.col('total_price')), 'total_amount']],
        raw: true,
      });
...
const getProfileDetails = async (req, res) => {
  try {
    ...
    if (!userProfile) {
      return res.status(404).json({ Status: 0, message: "The User profile not found" });
    }
