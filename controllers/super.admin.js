...
const SuperAdminSignIn = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ Status: 0, message: "Super admin not found" });

    const passwordMatch = await bcrypt.compare(password, user.password);
    if (!passwordMatch) return res.status(401).json({ Status: 0, message: "Invalid sign-in credentials" });

    const deviceDetail = { device_id, device_type, device_token, user_id: user.user_id };
    let deviceDetails = await db.Token.findOne({ where: { device_id, user_id: user.user_id } });
    ...
    return res.status(200).json({ Status: 1, message: "Sign-in successful", token, user_id: user.user_id });
  } catch (error) {
...
const createAdmin = async (req, res) => {
  try {
    if (req.userData.role !== "super_admin") return res.status(401).json({ Status: 0, message: "Unauthorize" });
    ...
    if (!!existingAdminWithUsername) {
      return res.status(409).json({ Status: 0, message: "Username already exists" });
    }
    ...
    fs.renameSync(`uploads/business_image/${imageUrlMedia}`, `uploads/business_image/${imageUrlWithExt}`);
...
const editAdmin = async (req, res) => {
  try {
    if (req.userData.role !== "super_admin") return res.status(401).json({ Status: 0, message: "Unauthorized" });
    ...
    const existingAdmin = await db.User.findOne({where: { user_id: admin_id, role: "admin", is_deleted: false },});
    if (!existingAdmin) return res.status(404).json({ Status: 0, message: "Admin not found" });
    
    const existingBussiness = await db.Business.findByPk(existingAdmin.business_id);
    
    if(email){
    const existingAdminWithEmail = await db.User.findOne({where: { email, user_id: { [Op.ne]: admin_id }, is_deleted: false },});
    if (existingAdminWithEmail) return res.status(409).json({ Status: 0, message: "Email already exists" });
    }
    
    if(username){
    const existingAdminWithUsername = await db.User.findOne({where: { username, user_id: { [Op.ne]: admin_id }, is_deleted: false },});
    if (existingAdminWithUsername) return res.status(409).json({ Status: 0, message: "Username already exists" });
    }
...
const getBusinessAdminList = async (req, res) => {
  try {
    if (req.userData.role !== "super_admin") return res.status(401).json({ status: 0, message: 'User is not authorized to access admin list' });
    ...
    const { count, rows: adminList } = await db.User.findAndCountAll({
      attributes: { exclude: ['password', 'updatedAt', 'otp', 'otp_created_at', 'is_verify', 'is_account_setup', 'is_deleted'] },
      where: { role: 'admin' },
    ...
    if (count === 0) return res.status(404).json({ status: 0, message: "No admin found" });

    const totalPages = Math.ceil(count / limit);
    res.status(200).json({ status: 1, message: "Admins fetched successfully", totalPages, adminList });
...
const getStaffList = async (req, res) => {
  try {
    ...
    if (req.query.role === "barista" || req.query.role === "supervisor") {
      ...
      const result = await db.User.findAndCountAll({
        attributes: { exclude: ['password', 'createdAt','updatedAt', 'otp', 'otp_created_at', 'is_verify', 'is_account_setup', 'is_deleted'] },
        where: whereCondition,
        distinct: true,
        order: [['createdAt', 'DESC']],
        limit: limit,
...
const getBusinessList = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 1, message: "bussiness fetched successfully", bussinessList });
...
const getBusinessTableList = async (req, res) => {
  try {
    if (req.userData.role !== "super_admin") return res.status(401).json({ status: 0, message: 'User is not authorized to access ' });

    const isBussiness = await db.Business.findByPk(req.query.business_id)
    if (!isBussiness) return res.status(404).json({ Status: 0, message: "bussines not found" });
    ...
    if (count === 0) return res.status(200).json({ Status: 0, message: "No table found", tables, bussiness_data:isBussiness });
    const totalPages = Math.ceil(count / limit);
    res.status(200).json({ Status: 1, message: "Get Table List Succesfully", totalPages, tables, bussiness_data:isBussiness });
  } catch (error) {
...
const getBusinessCategoryList = async (req, res) => {
  try {
    ...
    const totalPages = Math.ceil(count / limit);
    res.status(200).json({ Status: 1, message: "The Category get Succesfully", totalPages, categories, });
  } catch (error) {
...
const getBusinessItemList = async (req, res) => {
  try {
    ...
    if (!isCategory) return res.status(404).json({ status: 0, message: "category not found" });
    ...
    if (count === 0) return res.status(200).json({ Status: 0, message: "No ItemList found", data: rows});

    const totalPages = Math.ceil(count / limit);
    res.status(200).json({ Status: 1, message: "The ItemList get succesfully", totalPages, data: rows, });
  } catch (error) {
